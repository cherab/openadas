...
ion_interp_adas_cycle = {}
recom_interp_adas_cycle = {}
for i in range(neon.atomic_number):
    ac = i+1
    ...
    ion_interp_adas_cycle[i] = Interpolate2DCubic(10**ion_adas[neon][ac]["ne"], 10**ion_adas[neon][ac]["te"], np.log10(10**ion_adas[neon][ac]["rates"]))
    recom_interp_adas_cycle[ac] = Interpolate2DCubic(np.log10(10**recom_adas[neon][ac]["ne"]), np.log10(10**recom_adas[neon][ac]["te"]), np.log10(10**recom_adas[neon][ac]["rates"]))
...
ion_balance_adas_cycle = np.zeros((neon.atomic_number+1, electron_temperatures.shape[0]))
...
for j, te in enumerate(electron_temperatures):
    ...
    ion_balance_adas_cycle[:, j] = solve_ion_balance(PerCm3ToPerM3.inv(electron_density), te, ion_interp_adas_cycle, recom_interp_adas_cycle, True)
...
if False:
    ...
    for i in range(neon.atomic_number+1):
        try:
            ...
            ionisation_rates_adas_cycle = [Cm3ToM3.to(10 ** ion_interp_adas_cycle[i](np.log10(PerCm3ToPerM3.inv(electron_density)), np.log10(x))) for x in electron_temperatures]
            ...
            ax.loglog(electron_temperatures, ionisation_rates_adas_cycle, 'x', color=tmp[0].get_color())
    ...
    ax.plot([], [], "kx",label="adas cycle normal->log10 interp.-> normal")
    ...
    for i in range(1, neon.atomic_number+1):
        try:
            ...
            recombination_rates_adas_cycle = [Cm3ToM3.to(10 ** recom_interp_adas_cycle[i](np.log10(PerCm3ToPerM3.inv(electron_density)), np.log10(x))) for x in electron_temperatures]
            ...
            ax.loglog(electron_temperatures, recombination_rates_adas_cycle, 'x', color=tmp[0].get_color(), alpha = 0.5)
    ...
    for i in range(neon.atomic_number+1):
        ...
        ax.semilogx(electron_temperatures, ion_balance_adas_cycle[i, :], 'x', color=tmp[0].get_color(), alpha = 0.5)
